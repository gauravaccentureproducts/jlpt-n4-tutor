...
def fix_moji_source_lemma_prefix() -> None:
    md_path = KB / 'moji_questions_n4.md'
    ...
    for old, new in REPLACEMENTS:
        if old in text:
            ...
            changes.append(f'§2.4 moji_questions_n4.md: dropped lemma-prefix → {new[:30]}...')
...
def fix_moji_source_2_1() -> None:
    ...
    if old_q35 in text:
        ...
        changes.append('§2.1 moji_questions_n4.md Q35: 町→まち')
    ...
    if old_q95 in text:
        ...
        changes.append('§2.1 moji_questions_n4.md Q95: 八百屋→みせ')
    md_path.write_text(text, encoding='utf-8')
...
def fix_dokkai_exception_register() -> None:
    ...
    if added:
        ...
        if isinstance(doc, list):
            ...
            if not any('moji-and-source audit §2.2' in (line or '') for line in doc):
                ...
                data['_doc'] = doc
        save_json(p, data)
    # Also update KnowledgeBank/dokkai_questions_n4.md header per the
    # exception register's own instruction ("update KnowledgeBank/
    # dokkai_questions_n4.md header"). Append to the exception list there.
    md = KB / 'dokkai_questions_n4.md'
    if md.exists() and added:
        ...
        if '2026-05-03 audit §2.2' not in text:
            ...
            changes.append('§2.2 dokkai_questions_n4.md: header note appended')
...
def fix_goi_3_1_reverts() -> None:
    ...
    md_path = KB / 'goi_questions_n4.md'
    text = md_path.read_text(encoding='utf-8')
    old_q58 = 'きのう 早く ねました。'
    ...
    if old_q58 in text:
        ...
        md_path.write_text(text, encoding='utf-8')
        changes.append('§3.1 goi_questions_n4.md Q58: 早く→はやく (real correct-answer violation)')
...
def fix_bunpou_4_1() -> None:
    md_path = KB / 'bunpou_questions_n4.md'
    text = md_path.read_text(encoding='utf-8')
    # The audit cites Q19's rationale: "熱がある (have a fever)."
    if '熱がある' in text:
        text = text.replace('熱がある', 'ねつが ある')
        md_path.write_text(text, encoding='utf-8')
        changes.append('§4.1 bunpou_questions_n4.md: 熱がある→ねつが ある')
    elif '熱' in text:
