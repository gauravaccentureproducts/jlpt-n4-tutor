...
Bug 8 (filename rename of authentic_extracted_n4.md → externally_sourced_n5.md):
...
def fix_dokkai_md() -> None:
    ...
    if '初めて' in text:
        ...
        changes.append(f'Bug 2: dokkai_questions_n4.md - replaced {n_init}× 初めて → はじめて')

    # Bug 3: 急いで → いそいで
    if '急いで' in text:
        text = text.replace('急いで', 'いそいで')
        changes.append(f'Bug 3: dokkai_questions_n4.md - replaced {n_isogu}× 急いで → いそいで')

    # Bug 4: 書いた 人 → この 人; ひっしゃ → この 人
    if '書いた 人' in text:
        text = text.replace('書いた 人', 'この 人')
        changes.append(f'Bug 4: dokkai_questions_n4.md - replaced {n_kaita}× 書いた 人 → この 人')
    if 'ひっしゃ' in text:
        text = text.replace('ひっしゃ', 'この 人')
        changes.append(f'Bug 4: dokkai_questions_n4.md - replaced {n_hisha}× ひっしゃ → この 人')
...
def fix_vocab_pos_legend_header() -> None:
    p = KB / 'vocabulary_n4.md'
    text = read(p)
    OLD = '## Part-of-Speech Tags (added 2026-05-02)'
    NEW = '## Part-of-Speech Tags'
    if OLD in text:
        text = text.replace(OLD, NEW)
        changes.append('Bug 7: vocabulary_n4.md - stripped (added 2026-05-02) date stamp from POS legend header')
        write(p, text)
