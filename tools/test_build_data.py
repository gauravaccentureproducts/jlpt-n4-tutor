...
# ---------------------------------------------------------------------------
# Fixture: a small markdown file shaped like KnowledgeBank/kanji_n4.md.
...
def test_real_data_invariants() -> list[str]:
    """End-to-end: the real KnowledgeBank/kanji_n4.md must produce 106
    entries (matches data/n4_kanji_whitelist.json), all kanji glyphs are
    BMP characters, and no entry has the parenthetical-fragment shape.
    """
    failures = []
    kb = ROOT / "KnowledgeBank" / "kanji_n4.md"
    if not kb.exists():
        return [f"Smoke: KB file missing: {kb}"]
    readings = build_data.extract_kanji_readings(kb)
    if len(readings) != 106:
        failures.append(
            f"E2E regression: expected 106 kanji from KnowledgeBank/kanji_n4.md, "
