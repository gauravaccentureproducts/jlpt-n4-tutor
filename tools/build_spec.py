# -*- coding: utf-8 -*-
"""Build JLPT N4 Grammar Tutor Functional Specification as a .docx file.

Run from the repo root:
    python tools/build_spec.py

Overwrites: JLPT N4 Grammar Tutor - Functional Spec.docx
...
OUT_PATH = str(OUT_DIR / "JLPT N4 Grammar Tutor – Functional Spec.docx")
...
title_run = title_p.add_run("Functional Specification - JLPT N4 Grammar Tutor & Test")
...
p("Target learner: late-N5 (wrapping up N5; preparing for JLPT N4 exam).")
p("Live site: https://gauravaccentureproducts.github.io/jlpt-n4-tutor/")
...
bullet("**Service worker `jlpt-n4-tutor-v24`** - stale-while-revalidate for the shell (HTML/CSS/JS) and cache-first for content (data/audio/locales).")
...
bullet("**`data/n4_kanji_readings.json`** - 35 wrong-primary readings fixed (本 もと→ほん, 時 とき→じ, 月 つき→がつ, 学 まな→がく, 人 ひと→にん, etc.) so auto-furigana renders the N5-context-most-common reading.")
...
bullet("**§4.4 NEW Canonical pattern source** - locked to local file `KnowledgeBank/grammar_n4.md` (23 categories, exhaustive), not an external website.")
...
bullet("**§4.7 NEW N5 vocabulary whitelist** - content lint enforces scope using `KnowledgeBank/vocabulary_n4.md`.")
bullet("**§4.8 NEW N5 kanji whitelist** - furigana rule and content lint enforce scope using `KnowledgeBank/kanji_n4.md`.")
...
bullet("Teaches all JLPT N4 grammar patterns in a structured Chapter 1 format (TOC → meaning → form/connection → explanation → examples → common mistakes → simple Japanese meaning).")
...
bullet("**Learner (Krivi):** Wrapping up N5; preparing for the JLPT N4 exam. Already knows most N5 kanji visually. Default mode hides furigana on N5 kanji.")
bullet("**Tutor / Admin (You):** Maintains grammar content + question bank in source-of-truth files (`KnowledgeBank/grammar_n4.md`, `KnowledgeBank/vocabulary_n4.md`, `KnowledgeBank/kanji_n4.md`, `/data/grammar.json`, `/data/questions.json`). Reviews progress.")
...
h2("4.1 JLPT N4 Scope Constraints")
bullet("All lesson examples and test content must use only N5-scope vocabulary (per `KnowledgeBank/vocabulary_n4.md`) and N5-scope kanji (per `KnowledgeBank/kanji_n4.md`).")
...
bullet("The canonical pattern coverage list is the local file **`KnowledgeBank/grammar_n4.md`** (in this directory) - 23 categories, exhaustive at N5 level.")
...
bullet("Every pattern listed in `KnowledgeBank/grammar_n4.md` MUST have a matching entry in `/data/grammar.json`. A coverage-check script (`tools/check_coverage.py`) enforces this - fails commit if a pattern is missing.")
...
bullet("`KnowledgeBank/vocabulary_n4.md` is the canonical N5 vocabulary scope list.")
bullet("Generated artifact `/data/n4_vocab_whitelist.json` is consumed by the content lint script.")
...
bullet("`KnowledgeBank/kanji_n4.md` is the canonical N5 kanji scope list.")
bullet("Generated artifact `/data/n4_kanji_whitelist.json` is consumed by the furigana renderer and the lint script.")
...
bullet("TOC page lists all N5 grammar patterns grouped by category (per `KnowledgeBank/grammar_n4.md`'s 23 categories).")
...
bullet("10 questions, sampled across the 23 categories of `KnowledgeBank/grammar_n4.md` with priority on the highest-frequency categories (copula, particles, verbs, adjectives, te-form, tai-form, counters, conjunctions, demonstratives, question words).")
...
bullet("**FR-L2:** System shall render TOC grouped by category (per `KnowledgeBank/grammar_n4.md`'s 23 categories), in declared order.")
...
bullet("**FR-L5:** System shall enforce that every pattern in `KnowledgeBank/grammar_n4.md` has a matching entry in `/data/grammar.json`. The `tools/check_coverage.py` script verifies this and exits non-zero if any pattern is missing.")
...
bullet("**FR-P4:** Use the LocalStorage key namespace `jlpt-n4-tutor:*` to avoid collisions.")
...
bullet("Source-of-truth files: `KnowledgeBank/grammar_n4.md`, `KnowledgeBank/kanji_n4.md`, `KnowledgeBank/vocabulary_n4.md`, `KnowledgeBank/sources.md` (human-readable).")
bullet("App-consumed files: `/data/grammar.json`, `/data/questions.json`, `/data/n4_kanji_whitelist.json`, `/data/n4_vocab_whitelist.json`.")
bullet("`tools/build_data.py` - one-time conversion script (run by author after editing .md files). Generates JSON whitelists from kanji_n4.md and vocabulary_n4.md.")
bullet("`tools/check_coverage.py` - verifies every pattern in grammar_n4.md has an entry in grammar.json.")
...
code_block('''/JLPT/N5/                        # repo root for this app
  ...
  /data/
    ...
    n4_kanji_whitelist.json      # generated from kanji_n4.md
    n4_vocab_whitelist.json      # generated from vocabulary_n4.md
  /tools/
  ...
  KnowledgeBank/grammar_n4.md                  # source-of-truth pattern catalog
  KnowledgeBank/kanji_n4.md                    # source-of-truth N5 kanji list
  KnowledgeBank/vocabulary_n4.md               # source-of-truth N5 vocab list
  KnowledgeBank/sources.md                     # reference / authority documentation
  JLPT N4 Grammar Tutor – Functional Spec.docx   # this file
  README.md''')
...
bullet("TOC lists every pattern in `KnowledgeBank/grammar_n4.md`, grouped by its 23 categories.")
...
bullet("`tools/check_coverage.py` exits 0 - every pattern in `KnowledgeBank/grammar_n4.md` has a matching `/data/grammar.json` entry.")
...
bullet("LocalStorage key namespace: `jlpt-n4-tutor:*`.")
...
# canonical "JLPT N4 Grammar Tutor – Functional Spec.docx" but
# is visibly distinct in a directory listing.
# ============================================================
CONSOLIDATED_PATH = str(OUT_DIR / "JLPT N4 — Consolidated Spec.docx")
