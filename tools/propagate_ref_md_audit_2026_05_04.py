...
What this script does:
  ...
  1. data/kanji.json — sync 後 kun ordering with kanji_n4.md.
     入 was already in sync. No JSON-level scope-flag mirror needed
     (the [N4+] notes are MD-only annotations).

  2. data/n4_kanji_readings.json — sync 後 kun ordering same.
...
# =====================================================================
# 1 + 2. 後 kun ordering in kanji.json + n4_kanji_readings.json
# =====================================================================
def sync_ato_kun() -> None:
    OLD_KUN = ['のち', 'うし', 'あと']
    NEW_KUN = ['うし', 'あと', 'のち']  # mirrors kanji_n4.md update
    ...
    p2 = ROOT / 'data' / 'n4_kanji_readings.json'
    r = load(p2)
    if isinstance(r.get('後'), dict) and r['後'].get('kun') == OLD_KUN:
        r['後']['kun'] = NEW_KUN
        save(p2, r)
        changes.append('n4_kanji_readings.json 後: kun reordered うし/あと/のち')
