...
Bug 2 (MED): 40 whitelist entries don't appear as form/reading in
             ...
             vocabulary_n4.md by extracting all word-form tokens. The
             whitelist is intentionally a SUPERSET of vocab.json forms:
               ...
               - 30 recognition-only items (genuine N5 vocab tokens
                 used in vocabulary_n4.md gloss/example text but never
...
# =====================================================================
README_TEXT = """# n4_vocab_whitelist.json — Design

This whitelist is **generated** from `KnowledgeBank/vocabulary_n4.md` by
...
`vocabulary_n4.md` lists certain words under a single multi-form entry like
...
| Whitelist form | vocab.json canonical | Multi-form entry in vocabulary_n4.md |
|---|---|---|
...
These tokens appear in `vocabulary_n4.md` (extracted by build_data.py) and
...
## Maintenance
  ...
  - **Adding a new word to N5 scope**: edit `vocabulary_n4.md`, then run
    `python tools/build_data.py` to regenerate `n4_vocab_whitelist.json`.
  ...
  - **Removing a deprecated form**: edit `vocabulary_n4.md` to remove the
...
def write_whitelist_readme() -> None:
    p = DATA / 'n4_vocab_whitelist_README.md'
    if p.exists() and p.read_text(encoding='utf-8') == README_TEXT:
        return  # idempotent
    p.write_text(README_TEXT, encoding='utf-8')
    changes.append('Bug 2: wrote data/n4_vocab_whitelist_README.md (design rationale)')
