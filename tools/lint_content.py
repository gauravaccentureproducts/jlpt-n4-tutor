...
Two checks:
  1. Kanji scope     -- any kanji NOT on data/n4_kanji_whitelist.json (errors).
  2. Vocab scope     -- kanji-bearing word-tokens NOT in data/n4_vocab_whitelist.json
...
def main() -> int:
    kanji_path = ROOT / "data" / "n4_kanji_whitelist.json"
    vocab_path = ROOT / "data" / "n4_vocab_whitelist.json"

    if not kanji_path.exists() or not vocab_path.exists():
        print(
            "ERROR: data/n4_kanji_whitelist.json or n4_vocab_whitelist.json missing. "
